...
import { HomeIcon, MapPinIcon, CalendarIcon, MessageCircleIcon, UserIcon } from 'lucide-react';
export function BottomNav() {
  ...
  return <div className={`fixed bottom-0 left-0 right-0 ${isDarkPage ? 'bg-[#5A5A5A]/95 backdrop-blur-xl border-t border-white/10' : 'bg-white/95 backdrop-blur-xl border-t border-gray-200/50'} px-4 pb-safe z-[9999] transition-colors`}>
      <div className="flex justify-around items-center h-20 max-w-2xl mx-auto">
        {tabs.map(tab => {
        const Icon = tab.icon;
        const isActive = location.pathname === tab.path;
        return <button key={tab.id} onClick={() => navigate(tab.path)} className="flex flex-col items-center justify-center gap-1 min-w-[60px] transition-all relative active:scale-95">
              {/* Active indicator with white glow - consistent across all pages */}
