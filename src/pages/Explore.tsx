...
import { SearchIcon, CameraIcon, Flame, Dumbbell, Coffee, UtensilsCrossed, Scissors, Sparkles, Dog, Store, LucideIcon } from 'lucide-react';
...
export function Explore() {
  ...
  const exploreCategories = [
    ...
    { id: 'gyms', name: 'Gyms & Wellness', icon: Dumbbell },
    { id: 'salons', name: 'Salons & Bars', icon: Scissors },
    { id: 'beauty', name: 'Beauty', icon: Sparkles },
    { id: 'pets', name: 'Pets', icon: Dog },
    { id: 'local', name: 'Local Shops', icon: Store }
  ];
  ...
  return (
    <div className="min-h-screen bg-mapleon-gray pb-24 overflow-x-hidden">
      {/* Header */}
      <div className="bg-white px-4 sm:px-6 pt-12 pb-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-14 py-3.5 sm:py-4 rounded-2xl bg-mapleon-gray border-none focus:outline-none focus:ring-2 focus:ring-mapleon-coral/30 transition-all"
          />
          <button
            onClick={handleCameraSearch}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-2 active:scale-95 transition-transform"
          >
          ...
          {exploreCategories.map(category => {
            ...
            return (
              <button
                ...
                className={`px-4 sm:px-5 py-2.5 sm:py-3 rounded-full whitespace-nowrap transition-all font-medium active:scale-95 flex items-center gap-2 ${
                  selectedCategory === category.id
                    ? 'bg-white shadow-md text-mapleon-slate'
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          {discoveryItems.map((item, index) => (
            <div key={index} className={index === 0 ? 'col-span-2' : ''}>
              <DiscoveryCard {...item} onClick={() => navigate('/business/1')} />
            </div>
          ))}
        </div>
      </div>
